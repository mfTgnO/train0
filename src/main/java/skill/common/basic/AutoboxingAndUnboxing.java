...
Although you add the int values as primitive types, rather than Integer objects, to li, the code compiles. Because li is a list of Integer objects,
not a list of int values, you may wonder why the Java compiler does not issue a compile-time error.
...
Autoboxing and unboxing lets developers write cleaner code, making it easier to read. The following table lists the primitive types and their corresponding wrapper classes,
...
//        test_i();
    ...
    private static void test_ii(){
        List<Integer> li = new ArrayList<>();
        for (int i = 1; i < 50; i += 2)
            li.add(i);
    }

    private static void test_iii(){
        List<Integer> li = new ArrayList<>();
        for (int i = 1; i < 50; i += 2)
            li.add(Integer.valueOf(i));
    }

    private static int sumEven_i(List<Integer> li) {
        int sum = 0;
        for (Integer i: li)
            if (i % 2 == 0)
                sum += i;
    ...
    private static int sumEven_ii(List<Integer> li) {
        int sum = 0;
        for (Integer i : li)
            if (i.intValue() % 2 == 0)
                sum += i.intValue();
